...
class Command(BaseCommand):
    help = "My shinny new management command"
    
    #def add_arguments(self,parser):
    #    parser.add_arguments('count', nargs=1, type=int)
    ...
    def make_universities(self):
        university_names = (
            'MIT', 'MGU', 'CalTech', 'KPI', 'DPI', 'PSTU','Cornell','Stanford','CalU','UCLA'
        )
        self.universities =[]
        ...
        for student_id in Student.objects.values_list('pk', flat = True):
            ...
            for _ in range(random.randint(1, 10)):
                ...
                if index not in courses_already_linked:
                    ...
                    continue
                stud_courses.append(ThroughModel(student_id = student_id,course_id=self.courses[index].pk))
